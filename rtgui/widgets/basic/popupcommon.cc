    ...
    selected(-1)  // -1 means that the button is invalid
{
    ...
    setExpandAlignProperties(imageContainer, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);
    ...
    if (!label.empty()) {
        ...
        setExpandAlignProperties(buttonLabel, false, false, Gtk::ALIGN_START,
                                 Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(buttonGroup, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(buttonImage, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(arrowButton, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_FILL);
    arrowButton->add(*arrowImage);  //menuSymbol);
    arrowImage->show();
    buttonGroup->attach_next_to(*arrowButton, *button, Gtk::POS_RIGHT, 1, 1);
    arrowButton->signal_button_release_event().connect_notify(
        sigc::mem_fun(*this, &PopUpCommon::showMenu));
...
bool PopUpCommon::addEntry(const Glib::ustring& fileName,
...
bool PopUpCommon::insertEntry(int position,
                              ...
                              Gtk::RadioButtonGroup* radioGroup)
{
    ...
    bool success = insertEntryImpl(position, iconName, Glib::RefPtr<const Gio::Icon>(),
                                   image, label, radioGroup);
    if (!success && image) {
        delete image;
    }
    return success;
...
bool PopUpCommon::insertEntryImpl(int position,
                                  const Glib::ustring& iconName,
                                  const Glib::RefPtr<const Gio::Icon>& gIcon,
                                  RTImage* image,
                                  const Glib::ustring& label,
                                  Gtk::RadioButtonGroup* radioGroup)
{
    if (label.empty() || position < 0 || position > getEntryCount()) return false;
    ...
    newItem->signal_activate().connect(
        sigc::bind(sigc::mem_fun(*this, entrySelectedFunc), newItem));
...
void PopUpCommon::changeImage(const Glib::ustring& iconName,
                              const Glib::RefPtr<const Gio::Icon>& gIcon)
{
    if (!iconName.empty()) {
...
void PopUpCommon::entrySelected(int i)
{
    ...
    if (setSelected(posToIndex(i))) messageChanged(posToIndex(selected));
...
bool PopUpCommon::setSelected(int entryNum)
{
    ...
    if (entryNum < 0 || entryNum > ((int)images.size() - 1) || (int)entryNum == selected)
    {
        ...
        auto radioMenuItem =
            dynamic_cast<Gtk::RadioMenuItem*>(menu->get_children()[entryNum]);
...
{
    if (event->button == 1) {
        menu->popup_at_widget(arrowButton, Gdk::GRAVITY_SOUTH_WEST,
